...
#define BAUD	57600	// Baud rate (9600 is default)
    ...
    #define PIN_LED       LED_BUILTIN	// 13 : Builtin LED
    ...
    #define PIN_LED       LED_BUILTIN	// 13 : Builtin LED
#elif defined(ARDUINO_ch55x)
    // WCH CH552/CH554
#else
    #error Unsupported board selection.
#endif

#if defined(ARDUINO_ch55x)
    // WCH CH552/CH554
    #define PIN_LED       TIN2  //P1.4
    #define PIN_MCLR      TIN3  //P1.5
    #define PIN_DAT       TIN4  //P1.6
    #define PIN_CLK       TIN5  //P1.7

    #define LED_BIT       4
    #define ISP_MCLR_BIT  5
    #define ISP_DAT_BIT   6
    #define ISP_CLK_BIT   7

    #define ISP_MCLR(v)   PIN_MCLR = v
    #define ISP_MCLR_OUT  P1_DIR_PU |= _BV(ISP_MCLR_BIT)

    #define ISP_DAT(v)    PIN_DAT = v
    #define ISP_DAT_V     PIN_DAT
    #define ISP_DAT_IN    P1_DIR_PU &= ~(_BV(ISP_DAT_BIT))
    #define ISP_DAT_OUT   P1_DIR_PU |= _BV(ISP_DAT_BIT)

    #define ISP_CLK(v)    PIN_CLK = v
    #define ISP_CLK_IN    P1_DIR_PU &= ~(_BV(ISP_CLK_BIT))
    #define ISP_CLK_OUT   P1_DIR_PU |= _BV(ISP_CLK_BIT)

    #define LED(v)        PIN_LED = v
    #define LED_OUT       P1_DIR_PU |= _BV(LED_BIT)
    ...
    #define LED(v)        digitalWrite(PIN_LED, v)
    #define LED_OUT       pinMode(PIN_LED, OUTPUT)
#endif

#define ISP_CLK_DELAY  1
#define DELAY3 delayMicroseconds(ISP_CLK_DELAY * 3)
#define DELAY  delayMicroseconds(ISP_CLK_DELAY)
...
int rx_state = 0;
int rx_message_ptr;
unsigned char rx_message[180];
unsigned int flash_buffer[152];
...
#if defined(ARDUINO_ch55x)
    P1_MOD_OC &= ~(_BV(LED_BIT) | _BV(ISP_MCLR_BIT) | _BV(ISP_DAT_BIT) | _BV(ISP_CLK_BIT));  // P1.4,5,6,7: push-pull
#else
    Serial.begin(BAUD);
//    while (!Serial);
#endif
...
uint8_t usart_rx_b(void)
{
    return (unsigned char)USBSerial_read();
}
...
// #define fw_pp_ops_delay(n) do { uint8_t count = (n); while (0 < count--) delayMicroseconds(10); } while (0)
